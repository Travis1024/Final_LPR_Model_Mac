...
def cnn_train_green():
    ...
    for i in range(n):
        ...
        label = [char_dict[name] for name in pic_name[i][0:8]]  # 图片名前7位为车牌标签
        X_train.append(img)
        y_train.append(label)
    X_train = np.array(X_train)
    y_train = [np.array(y_train)[:, i] for i in range(8)]  # y_train是长度为7的列表，其中每个都是shape为(n,)的ndarray，分别对应n张图片的第一个字符，第二个字符....第七个字符
    ...
    Output = [layers.Dense(65, activation='softmax', name='c%d' % (i + 1))(x) for i in range(8)]  # 7个输出分别对应车牌7个字符，每个输出都为65个类别类概率
...
def cnn_predict_green(cnn, Lic_img):
    ...
    for lic in Lic_img:
        ...
        lic_pred = np.array(lic_pred).reshape(8, 65)  # 列表转为ndarray，形状为(7,65)
        if len(lic_pred[lic_pred >= 0.5]) >= 5:  # 统计其中预测概率值大于80%以上的个数，大于等于4个以上认为识别率高，识别成功
            chars = ''
